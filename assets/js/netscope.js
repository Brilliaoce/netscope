      ...
      switch (layertype) {
        ...
        case "convolutiondepthwise":
          ...
          if (layertype === "convolution") {
            ...
            //memory
            d.mem.param = (kernel_w * kernel_h) * d.chIn * d.chOut / group + has_bias * d.chOut; // convolutiondepthwise
          } else {
  ...
  renderTable() {
    ...
    if (do_variants_analysis) {
      ...
      for (k = 0, len1 = detail.length; k < len1; k++) {
        entry = detail[k];
        if (!(entry.type === "Convolution" || entry.type === "ConvolutionDepthwise" || entry.type === "Concat" || entry.type === "SoftmaxWithLoss" || entry.type === "innerproduct")) {
          continue;
        }
